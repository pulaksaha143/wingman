...
# ─── STREAMLIT CONFIG ───────────────────────────────────────────────────
st.set_page_config(
    page_title="Wingman (Experiment 2)",
    page_icon="🥂",
    layout="centered",
...
local_model_path = "models/Qwen2.5-3B-Instruct"
if not os.path.exists(local_model_path):
    ...
    snapshot_download(repo_id="Qwen/Qwen2.5-3B-Instruct", local_dir=local_model_path)

model_path = local_model_path
active_adapter = (
    "experiment2/outputs2/qwen2.5-3b-instruct-wingman-lora/adapters"
    if os.path.exists("experiment2/outputs2/qwen2.5-3b-instruct-wingman-lora/adapters")
    else None
)
temperature = 0.7
max_tokens = 256
...
if "pending_target" in st.session_state:
    ...
    if (
        ...
        if curr_mode == "JUDGE":
            ...
            # Let the user supply their own prefix, just append the nsfw tag
